...
Note that `i18n/res` is a **symlink** to `src/src/androidMain/res`, so
`i18n/res/values/strings.xml` and `src/src/androidMain/res/values/strings.xml` are the
...
The `:src:generateI18nFiles` Gradle task reads these XML files on every build and generates:

* `strings_<lang>.bin` --> one binary pack per locale (null-terminated UTF-8 strings,
  looked up by position), written to `src/build/generated/i18n/`.
...
1. Add the key to `i18n/res/values/strings.xml` (same file as
   `src/src/androidMain/res/values/strings.xml` --> add it once, not twice).
...
2. Add `<lang>` to `i18nLangs` in `src/build.gradle.kts`.
...
`src/src/commonMain/`, where they would collide with the generated copies and fail the
